...
class ProductVariationService
{
    ...
    {
        if ($request->id) {
            return ProductVariation::find($request->id)->bloodline()->get()->map(function ($variation) {
                $variation->flat_price = AppLibrary::flatAmountFormat($variation->price);
                return $variation;
            })->toTree();
        } else {
    ...
    {
        $productVariations = ProductVariation::with('media')->where('product_id', $product->id)->where('sku', '!=', null)->orderBy('parent_id', 'asc')->get()->map(function ($variation) use ($product) {
            $variation->product_name  = $product->name;
            $variation->category_name = $product->category->name;
            $variation->flat_price = AppLibrary::flatAmountFormat($variation->price);
            $variation->currency_price = AppLibrary::currencyAmountFormat($variation->price);
            return $variation;
        });
    ...
    public function downloadBarcode(ProductVariation $productVariation)
    {
        ...
        $productVariation->load(['ancestorsAndSelf.productAttribute', 'ancestorsAndSelf.productAttributeOption']);
        $productVariation->options = $this->nested($productVariation->ancestorsAndSelf->reverse());

        $optionsArray = collect($productVariation->options)
            ->map(fn($value, $key) => "{$key} / {$value}")
            ->values()
            ->toArray();

        $variationName = count($optionsArray) > 1
            ? implode(', ', $optionsArray)
            : implode('', $optionsArray);

        $barcodePath = str_replace(url('storage'), storage_path('app/public'), $productVariation->mediaUrl);
        $barcodeImage = Image::make($barcodePath)->resize(124, 40);
        $image = Image::canvas(216, 120, '#ffffff');

        $productNameLines = AppLibrary::barcodeText($product->name, 35, 2);
        $startY = 18;
        $lineHeight = 14;

        foreach ($productNameLines as $line) {
            $image->text($line, 10, $startY, function ($font) {
                $font->file(public_path('fonts/Urbanist-SemiBold.ttf'));
                $font->size(11);
                $font->color('#000');
            });
            $startY += $lineHeight;
        ...
        $variationY = $startY + 2;
        $image->text($variationName, 10, $variationY, function ($font) {
            $font->file(public_path('fonts/Urbanist-Medium.ttf'));
            $font->size(10);
            $font->color('#000');
        });

        $categoryY = $variationY + 13;
        $image->text('Category: ' . $product->category->name, 10, $categoryY, function ($font) {
            $font->file(public_path('fonts/Urbanist-Medium.ttf'));
            $font->size(10);
            $font->color('#000');
        });

        $barcodeY = $variationY + 18;
        $image->insert($barcodeImage, 'top-left', 10, $barcodeY);

        $image->text('Price', 140, $barcodeY + 10, function ($font) {
            $font->file(public_path('fonts/Urbanist-Medium.ttf'));
            $font->size(10);
            $font->color('#000');
        });

        $image->text(AppLibrary::currencyAmountFormat($productVariation->price), 140, $barcodeY + 30, function ($font) use ($productVariation) {
            $font->file(public_path('fonts/Urbanist-Bold.ttf'));
            $font->size(strlen(AppLibrary::currencyAmountFormat($productVariation->price)) > 9 ? 10 : 16);
            $font->color('#000');
        });

        $image->text($productVariation->sku, 55, $barcodeY + 50, function ($font) {
            $font->file(public_path('fonts/Urbanist-Medium.ttf'));
            $font->size(10);
            $font->color('#000');
        });

        $fileName = 'variation-barcode-sticker-' . $product->id . '.jpg';
        $tempFilePath = storage_path('app/public/' . $fileName);
        file_put_contents($tempFilePath, $image->encode('jpg'));
