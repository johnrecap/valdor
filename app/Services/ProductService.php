...
class ProductService
{
    ...
    {
        $barcodePath = str_replace(url('storage'), storage_path('app/public'), $product->barcodeImage);
        ...
        $categoryY = $startY + 5;
        $categoryText = 'Category: ' . $product->category->name;
        $categoryLines = AppLibrary::barcodeText($categoryText, 35, 2); // wrap text based on length

        foreach ($categoryLines as $line) {
            $image->text($line, 10, $categoryY, function ($font) {
        ...
        $image->insert($barcodeImage, 'top-left', 10, 60);

        $image->text('Price', 140, 70, function ($font) {
            $font->file(public_path('fonts/Urbanist-Medium.ttf'));
            $font->size(10);
            $font->color('#000');
        });

        $image->text(AppLibrary::currencyAmountFormat($product->selling_price), 140, 90, function ($font) use ($product) {
            $font->file(public_path('fonts/Urbanist-Bold.ttf'));
            $font->size(strlen(AppLibrary::currencyAmountFormat($product->selling_price)) > 8 ? 10 : 16);
            $font->color('#000');
        });
        $image->text($product->sku, 55, 110, function ($font) {
            $font->file(public_path('fonts/Urbanist-Medium.ttf'));
            $font->size(10);
            $font->color('#000');
        });

        $fileName = 'barcode-sticker-' . $product->id . '.jpg';
                    ...
                    if ($request->min_price >= 0 && $request->max_price > 0) {
                        $query->where(function ($q) use ($request) {
                            $q->whereHas('variations')
                                ->whereBetween('variation_price', [$request->min_price, $request->max_price]);
                        })->orWhere(function ($q) use ($request) {
                            $q->whereDoesntHave('variations')
                                ->whereBetween('selling_price', [$request->min_price, $request->max_price]);
                        });
                    }
                })->paginate($perPage);
