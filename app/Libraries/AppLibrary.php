...
class AppLibrary
{
    ...
    public static function amountCheck($amount, $attr = 'price'): object
    {
        ...
        if ($amount == "") {
            ...
            $response['message'] = "This {$attr} field is required.";
        }
        ...
        if ($amount == 0) {
            if (!$response['status']) {
                ...
                $response['status'] = false;
                $response['message'] = "This {$attr} does not allow zero value..";
            }
        }
        if ($amount < 0) {
            if (!$response['status']) {
    ...
    public static function barcodeText($text, $maxCharsPerLine, $maxLines = 2)
    {
        $lines = explode("\n", wordwrap($text, $maxCharsPerLine, "\n"));
        if (count($lines) > $maxLines) {
            $finalText = array_slice($lines, 0, $maxLines - 1);
            $lastLine = substr($lines[$maxLines - 1], 0, $maxCharsPerLine - 3) . '...';
            $finalText[] = $lastLine;
            return $finalText;
        }
        return $lines;
