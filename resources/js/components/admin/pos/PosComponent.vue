<template>
    ...
    <form class="w-full mb-4" @submit.prevent="search">
      <div class="form-row">
        <div class="form-col-12 sm:form-col-6"
          ...
          <div class="group w-full flex items-center h-10 px-3 rounded-md border border-[#EFF0F6] bg-white">
  ...
  <div
    ...
    <div v-if="carts.length === 0" class="flex flex-col items-center justify-center">
    ...
    applyDiscount: function () {
      if (this.discountType === discountTypeEnum.FIXED) {
        if (this.subtotal < this.discount) {
          alertService.error(this.$t('message.discount_fixed_error_message'));
        } else {
        ...
        if (this.discount > 100) {
          alertService.error(this.$t('message.discount_error_message'));
        } else {
